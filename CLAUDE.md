...
powershell -File Scripts\BuildJolt.ps1            # builds ExternalPackages/jolt/lib/Release/Jolt.lib   (Linux: bash Scripts/BuildJolt.sh)
powershell -File Scripts\BuildJolt.ps1 -Configuration Debug   # lib/Debug/Jolt.lib with JPH_ENABLE_ASSERTS
```

### Project-Physics (Jolt rigid-body testbed — console, no window, self-proving)
```powershell
powershell -File Projects\Project-Physics\Build\ToolchainSequence.ps1 -Run                          # plane + falling boxes/spheres/capsules
powershell -File Projects\Project-Physics\Build\ToolchainSequence.ps1 -Run -RunArguments '--fixed','--seconds','20','--quiet'
bash Projects/Project-Physics/Build/ToolchainSequence.sh --run -- --fixed --seconds 20 --quiet     # Linux; exit 0 = proof passed
```
- Flags: `--seconds N` budget, `--fixed` deterministic 1/60 s Δτ, `--boxes/--spheres/--capsules N`, `--quiet`, `--trace out.csv`
...
### Project-Ocean (WebGPU large-body-of-water testbed — phases O1 cascaded-FFT surface + foam, O2 shallow-water shoal patch)
...
- Run: `powershell -File Projects\Project-Ocean\Build\ToolchainSequence.ps1 [-Proof|-Dispersion|-RunUp|-Check|-Port N|-NoBrowser|-Query '…']`
  or `bash Projects/Project-Ocean/Build/ToolchainSequence.sh [--port N] [--bind 0.0.0.0] [--check]` (port 8766; browser only, no build)
- Query string: `tier=gtx|rtx` (3 × 256² at 8/2/0.5 m + 256² shoal at 1 m · 4 × 512² at 8/2/0.5/0.125 m + 512² shoal at 0.5 m), `bands`, `size`,
  `wind`, `fetch`, `depth`, `swell`, `chop`, `angle`, `seed`, `foam=0|1`, `jthreshold`, `azgamma`, `foamdecay`, `foamrate`,
...
- Look (after the user's first GTX screenshots): the hull is OFF by default (`hull=1` / checkbox) — without a ship mesh its wake foam
  read as a random white cloud; foam blur 0.35 → 0.04 per tick (crest-shaped whitecaps stay crest-shaped), foam drifts downwind at
  2.5 % U₁₀, shaded through a two-octave value-noise lace (holes open as the energy fades) with a milky bubble halo; the beach has
  wind ripples in the normal, a wet band + swash film from the foam record, sky Fresnel on wet sand, irregular cusps (two spacings);
  `FoamRate` 2.5 → 16 (a texel is under a firing crest ≈ 0.3 s, so 2.5/s never got a whitecap past energy 0.2 = grey; 16 saturates
  it white); the proof's coverage is now the white fraction the lace draws, Σ min(1, 1.2 E) (≈ 4 % at 22 m/s; Monahan 14.5 %); tier 2 also whitens the swash tongue (water
  < 25 cm moving ≥ 0.3 m/s, in the solver and the shader) and the render adds a foredune `Backshore` behind the berm (scene 2
  only — the patch never reaches it); the shore camera defaults to looking downwind at the beach
- Proofs: spectrum (Σ|h̃0|² = ∫S dk² per band ±5 %), parseval (mean h² = Σ|ĥ|² ±2 %), dispersion (`scene=mode`, ω ±1 %, A ±2 %), finite,
  ...
  FNV trace hash; headless: `Scratchpad/OceanHeadlessRun.mjs`
- Roles: `OceanStructure.js` (tiers, bands, sea state, shoal/bed description), `SwellSolver.js` + `Shaders/SwellSolver.wgsl` (spectrum,
  evolution, radix-2 FFT, compose, proofs), `Shaders/FoamSolver.wgsl` (camera-following foam window; bores → foam, SWE-advected in the patch),
  `ShoalSolver.js` + `Shaders/ShoalSolver.wgsl` (Stelling–Duinmeijer staggered SWE: upwind face depth, flux-form advection with the
  converging-flow upwind switch, Manning, sponge relaxation toward the long bands, hull pressure, ShoalShift window, measure),
  `HorizonProjection.js/.wgsl` (warped VS grid, band fade, depth fade, patch height/slope, sand, shading), `Shaders/SeaStructure.wgsl`
  (shared uniforms `Sea` + `Shoal`, analytic `Bed`, `PatchWeight`, `ShallowWeight`, `DepthWeight`, `HullHead`), `GameExecution.js`, `TimingMetrics.js`
...
│   │   └── RigidBodySolver.h/.cpp  ← Jolt behind a JPH-free seam: Bring/CreateBody/Advance(Δτ)/QueryPoses/Retire; +Z up, m/kg/s
...
│   ├── jolt/          @2e28006e   ← Rigid-body physics; NOT header-only — Scripts/BuildJolt.ps1|.sh → static lib
│   │   └── lib/<Config>/          ← Deposit: Jolt.lib / libJolt.a  (generated, not committed; ISA+NDEBUG must match client)
...
│   │       └── TracingIndex.h
│   ├── Project-Physics/            ← Jolt rigid-body testbed (console; ground plane + drop scene ticked from main loop)
│   │   ├── Build/ToolchainSequence.ps1/.sh
│   │   └── Source/
...
│   ├── BuildJolt.ps1/.sh           ← cl.exe + lib.exe (g++ + ar) → Jolt.lib / libJolt.a  (no CMake; flags == client TUs)
