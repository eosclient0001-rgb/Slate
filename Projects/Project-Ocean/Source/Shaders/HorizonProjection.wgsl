...
struct Surface
{
    ...
    @location(2) Weights: vec4f,     // per-band vertex weights
...
fn SurfaceFragment(in: Surface) -> @location(0) vec4f
{
    ...
    for (var b = 0u; b < U.Grid.z; b++)
    {
        let pixelWeight = clamp(2.0 - footprint / (U.Spacing[b] * V.Screen.z), 0.0, 1.0);
        ...
        roughness2 += (1.0 - pixelWeight) * V.Slope[b];
    }
