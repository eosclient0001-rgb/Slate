...
| **O2 Shoal** ✅ (§6a) | Camera-centred SWE patch over a terrain height map (Stelling–Duinmeijer FV), depth blend with the bands, bores → foam, wakes from a moving box | mass conservation to 1e-4 over 60 s; run-up height vs the analytic solitary-wave benchmark used by Celeris; ms at 256²/512²/1024² | [6][12][13][14] |
...
## 6a. O2 delivered — the shoal patch (status, numbers, what the numbers mean)

**What was built** (`Projects/Project-Ocean`, scenes `open` · `shore` · `runup`; tier profiles GTX 256² at 1 m, RTX 512² at 0.5 m):

* `Shaders/ShoalSolver.wgsl` — non-linear shallow-water equations on a staggered (Arakawa-C) grid after **Stelling & Duinmeijer 2003** (the exactly-conserving half of [6]): forward–backward stepping (`ShoalVelocity` → `ShoalDepth`), **upwind face depth** for wetting/drying (nothing drains below zero; a face at rest takes the higher free surface as upwind so a wet cell next to dry sand can start to flow), **flux-form advection** with their switch — upwind velocities where the flow converges (bores: momentum-conserving, right bore speed and dissipation), centred elsewhere (smooth run-up: second order) — implicit Manning friction, a speed cap, a **sponge rim** that relaxes height and velocity toward the long spectral bands (tier-0 waves enter the patch and patch waves leave without reflecting), a **hull** as a moving surface pressure (shallow-water wake), a bore detector (surface falling along the flow with Fr > 0.7), `ShoalShift` when the camera-centred window moves, and `ShoalMeasure` (volume, max η, wet cells, non-finite, max wet bed, kinetic energy, bore sum, max |u|).
* `Shaders/SeaStructure.wgsl` — one analytic **bed** (plane beach through the shoreline, offshore plain, berm; the shore scene adds a longshore bar and beach cusps) shared by solver, foam and renderer; `PatchWeight` (rim blend), `ShallowWeight` (λ/depth ≥ 8 → the SWE owns that band), `DepthWeight` (bands fade where the water is shallower than ≈ 2–8 σ of the band — where they would have broken), `HullHead`.
* Coupling: `HorizonProjection` replaces the long bands by η = h + bed inside the patch (finite-difference slope joins the normal; dry cells draw as sand with a wet fringe; shallow water shows the sand through Beer–Lambert attenuation; view 5 = depth/sand/bores/rim debug), `FoamSolver` injects foam at bores, advects foam with the SWE current inside the patch, fades the spectral criteria where the bands fade, and keeps no foam on dry sand.

**Proofs** (SwiftShader, 2026-09-06; `?scene=runup&seconds=60&proof=1&fixed=1`, closed basin, no spectral sea, Synolakis' 1:19.85 beach, H/d = 0.0185, d = 4 m; `?scene=shore&wind=14&seconds=24&proof=1&fixed=1&size=128&bands=2&shoalsize=128&shoalcell=2`):

| Proof | Criterion | GTX profile (256² @ 1 m, Δx/d = 0.25) | RTX profile (512² @ 0.5 m, Δx/d = 0.125) |
|---|---|---|---|
| volume 🧮 | Σ h Δx² drift < 1e-4 over 60 s | ✅ drift 1.6e-7 (fp32 sum noise) over 3 600 ticks | ✅ drift 8.2e-8 over 3 600 ticks |
| run-up 🌊 | highest wet free surface vs Synolakis R/d = 2.831 √cot β (H/d)^{5/4} = 0.0861, within 5 % + 0.5 Δx/d | ✅ R/d 0.0758 (−11.9 %, tolerance ±18 %) — 1-D mirror predicts −12.0 % | ✅ R/d 0.0811 (−5.8 %, tolerance ±11 %) — mirror −5.2 % |
| shoal-finite 🔒 | no NaN/Inf, |u| ≤ cap, η bounded | ✅ 0 non-finite · max |u| 0.10 m/s · max η 0.06 m | ✅ 0 non-finite · max |u| 0.10 m/s · max η 0.06 m |
| bores → foam 🫧 | shore scene, wind ≥ 12 m/s: detector fires in the surf zone | ✅ fires from t ≈ 0.5 s; Σ bore 500 cell-units at 24 s (a bore line on the bar, view 5); foam coverage 20 % in the window (surf), spectral-only sea 3.1 % | — |

The run-up error is the scheme's first-order-in-Δx/d bias (`Scratchpad/ShoalMirrorRun.mjs`, a 1-D CPU mirror of the kernel: −12.0 % at Δx/d = 0.25, −5.2 % at 0.125, −1.7 % at 0.0625; with pure upwind advection it was −16 / −11 / −5.5 %, and the GPU matched both numbers), so the tolerance scales with Δx/d and the log prints the error so the convergence across the tiers is visible. Volume is conserved to round-off (the flux form is exactly conservative; the drift reported is fp32 summation noise over 65 k–262 k cells). Two things the first GPU runs taught that the plan did not know: (1) with a face's upwind side chosen by the sign of a *zero* velocity, the wet cell beside dry sand never starts to flow and the run-up is 0 — the face at rest must take the higher free surface as upwind; (2) the bore detector cannot be a slope × Froude test on a first-order scheme (nothing ever exceeded the thresholds) — the rise-rate criterion of Kennedy et al. 2000 (∂η/∂t ≥ 0.25…0.65 √(g h), decaying over 0.35 s) fires exactly where a bore forms on the bar.

**Coupling as built** — the spectral bands and the patch share the surface by depth: the handover depth is the peak wavelength / 8 (15 m at 14 m/s); deeper than 1.5 × that the bands own the wave and the patch is *relaxed toward them* (height and depth-averaged velocity √tanh(kd)/(kd) × the texture's a√(gk)), shallower than 0.5 × it the patch runs free and draws the bands it can resolve (λ ≥ 6 cells) in their place; the bands fade where the water is shallower than ≈ 2–8 σ of the band (where they would have broken); the rim relaxes fully. The first attempt — the patch forced only in a 12-cell rim, free inside — lost the swell on the way in (η 0.15 m at the shore from a 4.4 m sea, no bores): a 256 m patch is 2 wavelengths, and a first-order scheme will not carry a wave that far. The nudging fixed it (η 2.8 m, |u| 5 m/s in the surf, bores continuous).

**Cost** (SwiftShader software rasteriser in the sandbox — relative only; the GTX/RTX CSVs from the user's machines remain the real numbers): GTX profile ShoalVelocity + ShoalDepth ≈ 63 ms per tick on 256² (one sub-step: Δτ = 1/60 s satisfies Courant 0.5 at 20 m depth and 1 m cells), RTX profile ≈ 273 ms on 512² at 0.5 m — ≈ 1.0 µs per cell per sub-step on the CPU rasteriser, against ≈ 95 ms for one 256² FFT band pair on the same rasteriser, i.e. **the patch costs about two-thirds of one spectral band**. On a real GPU the kernels are bandwidth-bound (≈ 30 texel reads per cell in ShoalVelocity, 5 in ShoalDepth), so the 0.3–0.5 ms class of Fluid Flux [12] and the Godot beach [13] is the expectation for 256²–512²; the RTX profile needs 2 sub-steps only when the deepest water exceeds ≈ 5 m at 0.5 m cells, which the `SubSteps` plan derives from the bed. To be confirmed by `?scene=shore&perkernel=1` on the target hardware.

**Not in O2 (by design)**: dispersion-correct wakes (the Airy half of [6], RTX-only extension), spectrum-consistent wave particles [5], and terrain from a height-map texture (the bed is analytic here so the solver, foam and renderer agree exactly; swapping `Bed()` for a texture read is the engine-side step).

**First hands-on report (user, GTX, 2026-09-06): "the parameters didn't seem to change the ocean."** Two causes, both fixed:

1. *The controls were not wired to the running sea.* Wind, fetch, depth, swell and choppiness were only read by the Restart button; moving a slider changed its label and nothing else. Now every spectral control applies live: `SwellSolver.Reseed` re-runs `SpectrumInit` on the same lattice (h̃0 re-drawn from the new spectrum, the same `Seed` phases, the clock untouched) so the surface morphs to the new sea state in one tick instead of resetting; tier, scene and foam change the lattice and rebuild in place with the camera kept; Restart rebuilds with a fresh clock. Proofs recorded under the previous spectrum are discarded (`Host.Generation`), and a rebuild waits for the tick in flight so no destroyed buffer is submitted — the previous Restart raised `"SeaConstants" used in submit while destroyed` on every click.
2. *The fetch made the wind slider nearly inert at the low end.* Hasselmann's fetch-limited JONSWAP (ω̃p = 22 X̃^(−⅓), α = 0.076 X̃^(−0.22)) keeps growing the sea with fetch without limit, so at the default 200 km a 2 m/s breeze was given Hs 0.76 m and Tp 4.6 s — a 4 m/s "sea" of 1.4 m, 6 m/s 2.0 m — while the fully developed Pierson–Moskowitz sea for those winds is 0.09 / 0.34 / 0.77 m. Between 2 and 10 m/s the slider therefore moved Hs from 0.8 to 3.2 m (×4) where nature moves it from 0.1 to 2.1 m (×23), and the waves it added were long ones a 6 m camera cannot read. The effective fetch is now capped at full development, gF/U² = 2×10⁴ (Holthuijsen 2007 §6.3; the JONSWAP peak law meets ω̃p ≈ 0.85 there): 2 / 4 / 6 / 8 m/s now give Hs 0.12 / 0.53 / 1.21 / 2.15 m and Tp 1.6 / 3.3 / 4.9 / 6.5 s, with the UI reporting the capped fetch ("200 km → 73 km (sea fully developed at this wind)"). Above ≈ 9.4 m/s the 200 km default is still fetch-limited and nothing changed. Residual: γ = 3.3 at the cap overshoots PM's energy by ≈ 40 % (Hs 1.21 m vs 0.77 m at 6 m/s); the mature-sea γ → 1 blend is a one-line follow-up once the GTX images show whether the low-wind sea reads as too rough.

**First GTX screenshots (user, Turing, same day): "may need to work on the beach; and the foam; the foam looks rather random."** Read against the code, the four images say:

* *The random foam was the hull.* Open and shore scenes carried a Gaussian pressure patch 18 m ahead of the camera (radius 4 m, head 1.2 m) so the O2 coupling could be exercised without a ship; the water rising behind it as the user sails trips the rise-rate bore detector, which injects foam at 3 × bore — so every sail leaves a white cloud with no visible cause, and turning round shows a screen-filling one (image 2). The hull is now **off by default** (`hull=1` or the checkbox opts in; the tier-2 machinery is unchanged) until a ship mesh exists to explain it.
* *Whitecaps came out as grey blobs, not crest lines.* Three shading-side reasons, none in the physics: (1) the tent blur weight was 0.35 per tick, i.e. σ ≈ √(0.35 · 240) ≈ 9 m of diffusion over one 4 s lifetime — a 1–2 m wide crest line became a 20 m disc before it faded; now 0.04 (σ ≈ 3 m). (2) The foam was drawn as a solid sheet with a 4-texel hash speckle, so fading foam was a fading grey smear; it is now a coverage budget shaped by two octaves of value noise (`Lace`): fresh foam is solid, below ≈ 0.5 energy it breaks into streaks and holes, and a milky bubble halo (`bubbles`) sits under and around it — the War Thunder / Sea of Thieves lineage [B1][B2]. The pattern is band-limited by the pixel footprint so the horizon does not shimmer. (3) Foam sat exactly where it was born; it now drifts downwind at 2.5 % U₁₀ (wind + Stokes drift of the surface layer), so a dying whitecap streaks along the wind instead of hovering.
* *The beach was a flat tan card with a saw-tooth shoreline.* The cusps were one sine at 35 m with ± 8 m shoreline excursion (0.4 m / slope 0.05); now two incommensurate spacings (λ, 1.618 λ) at 0.3 m — horns and bays, not a saw. The sand gets wind ripples in the normal (0.3 m wavelength across the wind, faded with footprint), a saturated wet band 0.5 m above the waterline plus wet sand wherever the swash record (the foam memory, which the bores write) is fresh, a sky-Fresnel sheen on the wet sand, a foam-flecked swash film on the tongue, and a 7 m-scale tone variation so the distant beach has structure. Still open for O4: a real backshore (dune, vegetation line) and a texture-driven bed.
* Image 3 (run-up basin, depth slider at 500 m) is correct as rendered: the basin ignores the depth slider (d = 4 m is the benchmark), the sea is a mirror because the scene has no spectral sea, and the camera had been yawed along the shoreline.

*Second pass (own SwiftShader renders of the above, 128² bands).* The first pass fixed the shapes but not the brightness: at 8 m camera height no whitecap was visible at 14 or 22 m/s. The cause was the injection rate — a crest satisfies the −a_z ≥ 0.39 g / J < 0.6 criteria for ≈ 0.25 s at strength 0.2–0.5, so `FoamRate` 2.5/s charged a whitecap to energy 0.1–0.2 and the lace (which opens holes below 0.5) drew almost nothing. `FoamRate` is now 16/s (a texel sits under a firing crest for ≈ 0.3 s, so the fresh whitecap saturates: solid white, then lace) and the covered fraction is 1.2 × energy. The coverage statistic itself changed meaning: it was the area with any energy above 0.02, which counted grey tails the lace never draws (25 % "coverage" with almost nothing visible); it is now Σ min(1, 1.2 × energy) / texels — the white fraction the renderer paints, which is what Monahan's photographs counted. At 22 m/s that is ≈ 4 % against Monahan's 14.5 %; the 3.41 power law was fitted below ≈ 15 m/s and later data sets sit well under it at storm winds, so the gap is left as a tuning question (raise `FoamDecay` for longer streaks) rather than forced. Tier 2 whitens the swash tongue itself — water thinner than 25 cm moving faster than 0.3 m/s injects foam in the solver and is drawn white in the shader — so the waterline is the bright, ragged line a beach has rather than a colour boundary; beyond the patch the spectral foam fades with the bands and is zero on dry sand. The sand ripples are band-limited harder (footprint / 0.08 m) so they vanish before they alias into a texture from 40 m, and a rendering-only foredune (`Backshore`: 3–5 m ridge 10–90 m behind the berm, scene 2 only) gives the land a skyline. The shore scene's default camera now looks downwind, at the surf and the beach, instead of upwind at the open sea.
